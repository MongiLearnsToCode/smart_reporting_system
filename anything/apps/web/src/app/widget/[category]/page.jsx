...
function LogDetailCard({ log, onClick }) {
...
export default function WidgetDetailPage(props) {
  ...
  // Stats
  const totalAmount = logs.reduce(function (sum, l) {
    return (
      sum +
      (l.entities && l.entities.amount != null ? Number(l.entities.amount) : 0)
    );
  }, 0);
  const hasAmounts = logs.some(function (l) {
    return l.entities && l.entities.amount != null;
  });
  const currency =
    logs.find(function (l) {
  ...
  const conflicts = logs.filter(function (l) {
        ...
        <div>
          ...
          {isLoading ? (
            <div className="flex items-center justify-center py-20 text-zinc-600">
              <div
                className="h-6 w-6 rounded-full border-2 border-zinc-600 border-t-transparent"
                style={{ animation: "spin 0.8s linear infinite" }}
              />
            </div>
