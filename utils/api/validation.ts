...
// The plan a user is on is NOT a setting. It used to be one, which meant this
// endpoint would happily accept `{"tier":"pro"}` and grant it. Entitlement now
// comes from a Polar subscription, read server-side from Convex
// (convex/billing.ts) and never writable by the client.
